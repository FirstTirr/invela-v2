...
export default function ItemInstancePage() {
  ...
  const fetchInstances = async () => {
    try {
      setLoading(true);
      const [target, all] = await Promise.all([apiPerangkat.getById(itemId), apiItemInstance.getAll()]);
      setBasePerangkat(target);
      setUnitInstances(all.filter(i => Number(i.id_perangkat ?? i.perangkat_id) === itemId));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  useEffect(() => { if (itemId) fetchInstances(); }, [itemId]);
  ...
  const handleAddUnit = async () => {
    ...
    try {
      ...
      const lastCode = unitInstances.length ? unitInstances.reduce((max, c) => 
        (parseInt(c.kode_asset.replace(/\D/g, '')) || 0) > (parseInt(max.replace(/\D/g, '')) || 0) ? c.kode_asset : max
  ...
  const openHistory = async (unit: ItemInstance) => {
    ...
    setActionLoading(true);
    try {
      setHistoryList(await apiRiwayatPerbaikan.getByKodeAsset(unit.kode_asset));
    } finally {
      setActionLoading(false);
    }
  };
      ...
      <div className="space-y-6 font-sans text-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/kabeng/items')} className="p-2 border rounded-lg hover:bg-gray-50 shrink-0">
          ...
          <button disabled={actionLoading || loading} onClick={handleAddUnit} className="w-full sm:w-auto px-4 py-2 bg-primary text-white font-bold rounded-lg flex gap-2 items-center justify-center hover:opacity-90 disabled:opacity-50 transition-all">
            {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} Tambah Unit (+1)
          </button>
        </div>

        {/* Table Wrapper (Perbaikan Scroll HP) */}
                  ...
                  <tr key={u.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="p-3 sm:p-4 text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => openHistory(u)} className="p-2 text-primary hover:bg-primary/10 rounded-lg flex items-center gap-1 font-bold text-xs" title="Riwayat">
                          <History className="w-4 h-4 sm:w-5 sm:h-5" /> <span className="hidden sm:inline">Riwayat</span>
                        </button>
                        <button onClick={() => { setSelectedUnit(u); setEditForm({ kode_asset: u.kode_asset, status: u.status || 'aktif' }); setModalType('edit'); }} className="p-2 text-gray-600 hover:text-primary rounded-lg flex items-center gap-1 font-bold text-xs" title="Edit">
                          <Edit3 className="w-4 h-4 sm:w-5 sm:h-5" />
                        </button>
                        <button onClick={() => handleDelete(u.id, u.kode_asset)} disabled={deletingId === u.id} className="p-2 text-gray-400 hover:text-red-600 rounded-lg" title="Hapus">
          ...
          {!loading && unitInstances.length > 0 && (
            <div className="p-3 sm:p-4 bg-gray-50 border-t flex justify-between items-center text-xs">
              ...
              <div className="flex gap-2">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 border rounded-lg disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button>
                <span className="px-3 py-2 font-bold bg-primary text-white rounded-lg">{currentPage} / {totalPages}</span>
                <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 border rounded-lg disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
            <div className="bg-white rounded-2xl border shadow-xl w-full max-w-lg overflow-hidden">
              <div className="p-4 border-b flex justify-between items-center">
                ...
                <button onClick={() => setModalType(null)} className="p-1 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5" /></button>
              </div>
              ...
              {modalType === 'edit' ? (
                <form onSubmit={handleSaveEdit} className="p-4 sm:p-5 space-y-4">
                  ...
                  <div className="pt-3 flex justify-end gap-2 border-t">
                    <button type="button" onClick={() => setModalType(null)} className="px-4 py-2 font-bold text-gray-500 text-xs sm:text-sm">Batal</button>
                    <button type="submit" disabled={actionLoading} className="px-4 py-2 bg-primary text-white font-bold rounded-lg flex items-center gap-2 text-xs sm:text-sm">
                ...
                <div className="p-4 sm:p-5 max-h-[60vh] overflow-y-auto">
                  {actionLoading ? <div className="p-8 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto" /></div> : 
                    ...
                    <div className="space-y-3">
                      {historyList.map(h => (
                        <div key={h.id} className="p-3 border rounded-lg text-xs space-y-1">
                          <div className="flex justify-between font-bold"><span>{h.nama_teknisi}</span><span className="text-emerald-600">Rp {h.biaya.toLocaleString('id-ID')}</span></div>
                          <p className="text-gray-500">Perbaikan: {h.deskripsi_perbaikan}</p>
                          <span className="text-[10px] text-gray-400 block">{new Date(h.tanggal_perbaikan).toLocaleDateString('id-ID')}</span>
                        </div>
                      ))}
                    </div>
                  )}
