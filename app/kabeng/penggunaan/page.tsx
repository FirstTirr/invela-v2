...
import { apiLabor, apiPenggunaan, apiJurusan, Labor, Penggunaan } from '@/lib/api';
...
export default function PenggunaanLaborPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (userStr) {
        ...
        if (typeof jVal === 'number' || !isNaN(Number(jVal))) setUserJurusanId(Number(jVal));
        else if (typeof jVal === 'string') rawJurusan = jVal.toLowerCase().trim();
      }
      setUserRole(rawRole);
      if (userName && !pdfSignNama) setPdfSignNama(userName);

      const [dataLogs, dataLabor, dataJurusan] = await Promise.all([
        apiPenggunaan.getAll(), apiLabor.getAll(), apiJurusan.getAll()
      ]);

      const rawLogs = Array.isArray(dataLogs) ? dataLogs : [];
      const rawLabors = Array.isArray(dataLabor) ? dataLabor : [];
      const rawJurusans = Array.isArray(dataJurusan) ? dataJurusan : [];

      let matchedJName = rawJurusan;
      if (userJurusanId) {
        const found = rawJurusans.find((j: any) => Number(j.id) === userJurusanId);
      ...
      const myLabors = isNotAdmin ? rawLabors.filter((lab: any) => {
        ...
        return (userJurusanId && lJId === userJurusanId) || (matchedJName && lJName.includes(matchedJName));
      }) : rawLabors;
  ...
  const getLaborName = (log: any) => log.labor?.labor || log.labor?.nama_labor || log.Labor?.labor || '-';

  const filteredLogs = usageLogs.filter((log: any) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const tableData = filteredLogs.map((l: any, i) => [
      ...
      <div className="space-y-6 font-sans antialiased tracking-tight">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b border-surface-container pb-4">
          ...
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            <button onClick={exportExcel} className="flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-emerald-800 bg-emerald-100 hover:bg-emerald-200 rounded-lg cursor-pointer border border-emerald-300">
              <FileSpreadsheet className="w-4 h-4 text-emerald-700" /> Export Excel (.xlsx)
            </button>
            <button onClick={() => setIsPdfModalOpen(true)} className="flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-white bg-blue-900 hover:bg-blue-950 rounded-lg cursor-pointer shadow-xs">
              <FileText className="w-4 h-4 text-blue-200" /> Export PDF
            </button>
            <button onClick={fetchData} disabled={loadingLogs} className="flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-primary bg-primary/10 hover:bg-primary/20 rounded-lg disabled:opacity-50 cursor-pointer">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl border border-surface-container-high shadow-xs">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select value={selectedLabor} onChange={(e) => setSelectedLabor(e.target.value)} disabled={loadingLabor} className="w-full sm:w-auto px-3 py-2 text-sm border border-surface-container rounded-lg focus:outline-none focus:border-primary bg-white font-medium text-on-surface cursor-pointer">
              ...
              {laborList.map((item: any) => (
                <option key={item.id} value={item.labor || item.nama_labor}>{item.labor || item.nama_labor}</option>
              ))}
            </select>
          </div>
        </div>
                  ...
                  filteredLogs.map((log: any) => (
                    <tr key={log.id} className="hover:bg-surface-low/30 transition-colors">
                      ...
                      <td className="p-4 text-right">
                        <button onClick={() => handleDelete(log.id)} className="p-1.5 hover:bg-red-50 text-red-600 rounded-lg cursor-pointer" title="Hapus Log">
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-xl shadow-xl border border-surface-container max-w-md w-full overflow-hidden">
                <div className="flex items-center justify-between p-4 border-b border-surface-container">
                  ...
                  <button onClick={() => setIsPdfModalOpen(false)} className="p-1 text-outline hover:text-on-surface cursor-pointer"><X className="w-4 h-4" /></button>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 p-4 bg-surface-low border-t border-surface-container">
                  <button onClick={() => setIsPdfModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-outline hover:text-on-surface cursor-pointer">Batal</button>
                  <button onClick={generatePDF} className="flex items-center gap-2 px-5 py-2 text-sm font-bold text-white bg-blue-900 hover:bg-blue-950 rounded-lg cursor-pointer shadow-xs">
